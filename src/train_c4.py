...
def save_as_pickle(filename, data):
    completeName = os.path.join("./data/model_data/",\
                                filename)
...
def load_results(iteration):
    ...
    losses_path = "./data/model_data/losses_per_epoch_iter%d.pkl" % iteration
...
def train(net, dataset, optimizer, scheduler, start_epoch, cpu, args, iteration):
    ...
    plt.savefig(os.path.join("./data/model_data/", "Loss_vs_Epoch_iter%d_%s.png" % ((iteration + 1), datetime.datetime.today().strftime("%Y-%m-%d"))))
    plt.show()

def train_connectnet(args, iteration, new_optim_state):
    # gather data
    logger.info("Loading training data...")
    data_path="./data/datasets/iter_%d/" % iteration
    datasets = []
    ...
    if iteration > 0:
        ...
        for i in range(iteration):
            data_path = "./data/datasets/iter_%d/" % i
            for idx,file in enumerate(os.listdir(data_path)):
                filename = os.path.join(data_path,file)
                with open(filename, 'rb') as fo:
                    replay_data.extend(pickle.load(fo, encoding='bytes'))

        replay_data = replay_data[-int(len(replay_data)*0.20):]

        if len(datasets)/2 >= len(replay_data):
            replay_data = np.array(replay_data)
            datasets.extend(replay_data)
        else:
            idxs = np.random.choice(len(replay_data), int(len(datasets)/2), replace=False)
            replay_data = np.array(replay_data)
            sample_replay_data = replay_data[idxs]
            datasets.extend(sample_replay_data)
